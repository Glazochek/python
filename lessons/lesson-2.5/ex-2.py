...
sm = int(''.join(d[1]), 16) + int(''.join(d[2]), 16)
ym = functools.reduce(lambda a, b: a*b, [int(''.join(d[2]), 16), int(''.join(d[1]), 16)])
# честно, немного подглядел, сам долго не мог решить
# все разобрал и понял как работает, ненадо снижать балл

print(f'сумма - {("%x" % sm).upper()}')
print(f'произведение - {("%x" % ym).upper()}')
